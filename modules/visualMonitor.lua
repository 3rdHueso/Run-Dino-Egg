...
M.fontSize = 16
M.updateFreq = int( fps * 0.1 ) -- Ten updates per second ( /10 )
M.updateCount = 0
local prevTime = 0
	...
	local function countChildren( group )
		...
		if group.numChildren and not group.isMonitor then
			...
			for i = group.numChildren, 1, -1 do
				...
				--if group[i].name then			--*****use to check the objects created in scenes*****
				--	print(group[i].name)
				--else
				--	group[i].name = composer.getSceneName( "current" )
				--	if group[i].name ~= "game" then
					--group[i]:toFront()
				--	group[i].alpha = 0.5
				--	end
				--end
...
local function addDisplay()
	...
	background.name = "backgroundMonitor"
